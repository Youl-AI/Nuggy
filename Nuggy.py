...
# 🔥 분리한 로직 파일 임포트 (inference_utils.py가 같은 폴더에 있어야 합니다)
...
with st.sidebar:
    ...
    with st.expander("전문가 설정", expanded=True):
        ...
        # 👇 [수정됨] 최대값 0.5, 기본값 0.2로 변경 완료!
        NOISE_CUTOFF = st.slider("노이즈 제거 (Cutoff)", 
                                 min_value=0.0, 
                                 max_value=0.5, 
                                 value=0.2, 
                                 step=0.01,
                                 help="값이 클수록 배경이 깨끗해지지만, 0.3 이상이면 얇은 선이 지워질 수 있습니다.")
...
if model is None:
    ...
    if uploaded_file is not None:
        ...
        # 📂 파일이 업로드되지 않았을 때 보여줄 초기 화면 (갤러리 포함)
        ...
        with c1:
            ...
            if os.path.exists("./assets/example_fur.png"):
                ...
                st.warning("이미지 준비중 (assets/example_fur.png)")
        ...
        with c2:
            ...
            if os.path.exists("./assets/example_racket.png"):
                ...
                st.warning("이미지 준비중 (assets/example_racket.png)")
        ...
        with c3:
            ...
            if os.path.exists("./assets/example_web.png"):
                ...
                st.warning("이미지 준비중 (assets/example_web.png)")
